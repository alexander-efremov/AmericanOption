...
namespace CoreLib
{
    public class TecplotPrinter
    {
        ...
        {
            var t = tau * k;
            var name = string.Format("{0}_t={3}_nx={1}_hx={2}_a={4}_c={5}.dat", filename, data.Length, h, t, a, b);
        ...
        public void PrintXY(string filename, double t, double h, double[] data, int id, string mapName)
        {
            ...
            using (var writer = new StreamWriter(name, false))
            {
                ...
                for (var i = 0; i < data.Length; i++)
                {
                    ...
                    writer.WriteLine(
                        $"{(i * h).ToString("e8", CultureInfo.InvariantCulture)} {val.ToString("e8", CultureInfo.InvariantCulture)}");
                }
            }
        }

        [SuppressMessage("ReSharper", "UnusedMember.Global")]
        public void PrintXY(string filename, double t, double h, double[] data, int id, string mapName, string comment,
            int commentIndex)
        {
            var name = string.Format(
                "{0}_id_{6}_t={3}_nx={1}_hx={2}_a={4}_c={5}.dat",
        ...
        public void PriщntXY(string filename, double t, double h, Point[] data, int id, string mapName)
        {
            var name = string.Format(
                "{0}_id_{6}_t={3}_nx={1}_hx={2}_a={4}_c={5}.dat",
                filename,
        ...
        public void PrintXY(string filename, int k, string t, string h, double hd, double K, Point[] data,
            string mapName, string mapName2, bool printexct)
        {
            ...
            using (var writer = new StreamWriter(name, false))
            {
                ...
                if (printexct)
                    while (s < data[0].S)
                    {
                        var val = K - s;
                        writer.WriteLine(
                            $"{s.ToString("e8", CultureInfo.InvariantCulture)} {val.ToString("e8", CultureInfo.InvariantCulture)}");
                        s += hd;
                    }
        ...
        public void PrintXY(string filename, double t, double h, Point[] data, int id, string mapName, string comment,
            int commentIndex)
        {
            var name = string.Format(
                "{0}_id_{6}_t={3}_nx={1}_hx={2}_a={4}_c={5}.dat",
            ...
            using (var writer = new StreamWriter(name, false))
            {
                ...
                for (var i = 0; i < data.Length; i++)
                {
                    ...
                    writer.WriteLine(
                        $"{data[i].S.ToString("e8", CultureInfo.InvariantCulture)} {val.ToString("e8", CultureInfo.InvariantCulture)}");
                }
            }
        }
        ...
        {
            var name = string.Format("{0}_nx={1}_hx={2}_t={3}_a={4}_c={5}.dat", filename, exact.Length, h, t, a, b);
            using (var writer = new StreamWriter(name, false))
            {
                ...
                for (var i = 0; i < exact.Length; i++)
                    writer.WriteLine(
                        $"{(S0 + i * h).ToString("e8", CultureInfo.InvariantCulture)}  {exact[i].ToString("e8", CultureInfo.InvariantCulture)}");
                ...
                for (var i = 0; i < exact.Length; i++)
                    writer.WriteLine(
                        $"{(S0 + i * h).ToString("e8", CultureInfo.InvariantCulture)}  {numeric[i].ToString("e8", CultureInfo.InvariantCulture)}");
            }
        }
    }
}